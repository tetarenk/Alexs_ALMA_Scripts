...
slices.writeto(datadir+'PVdiag/pv_'+line+'.fits')


fitsfile = fits.open(datadir+'PVdiag/pv_'+line+'.fits')
hdu = fitsfile[0]
datapv=hdu.data
...
fitsfile18 = fits.open(datadir+'PVdiag/pv_18CO.fits')
hdu12 = fitsfile12[0]
hdu13 = fitsfile13[0]
hdu18 = fitsfile18[0]
datapv12=hdu12.data
...
fig,axes=plt.subplots()
gs=gridspec.GridSpec(3,3,width_ratios=[1,0.2,1])
gs2=gridspec.GridSpec(3,3,width_ratios=[1,0.1,1])
...
levels=np.array([1,2,3,3.5,4,5])*0.345
plt.contour(X,Y,Z,levels,colors='k')
...
levels=np.array([1,2,3,3.5,4,5])*0.2
plt.contour(X,Y,Z,levels,colors='k')
#ax1.set_ylabel('Velocity (km/s)')
#ax2.set_xlabel('Offset (arcsec)')
...
ax4 = plt.subplot(gs2[0:3, -1],projection=wmap1.celestial)
...
plt.savefig(datadir+'for_paper/pv_allCO.pdf',bbox_inches='tight')
plt.show()
